...
class ReportGenerator:
    ...
    def __init__(self, file_name: str, output_directory: str = "report", css_path: str = "") -> None:
        ...
        if not css_path:
            ...
            # If no custom css_path is provided, load the default styles.css
            # with importlib.resources.files("quantAnalytics", "styles.css") as css_file:
            #     self.css_path = str(css_file)
        else:
    ...
    def _init_report(self) -> None:
        """Create HTML header."""
        self._html_content += f"""<!DOCTYPE html>\n<html>\n<head>\n    <title>Backtest Report</title>\n    <link rel="stylesheet" href="{self.css_path}">\n</head>\n<body>"""

    def add_header(self, text: str, header: Header, css_class: Optional[str] = None) -> None:
    ...
    def add_paragraph(self, text: str, css_class: Optional[str] = None) -> None:
        """
        ...
        if css_class:
            ...
            self._html_content += f'<p class="{css_class}">{text}</p>\n'
        else:
            # Add a plain paragraph if no class is provided
            self._html_content += f"<p>{text}</p>\n"

    def add_unorderedlist_dict(self, items: Dict, bold_key: bool = True, css_class: Optional[str] = None) -> None:
        """
    ...
    def add_unorderedlist(self, items: list, css_class: Optional[str] = None) -> None:
        """
    ...
    def add_orderedlist(self, items: List, css_class: Optional[str] = None) -> None:
        if not isinstance(items, list):
            raise TypeError(f"Expected items to be a list, but got {type(items).__name__}")
    ...
    def add_table(
        ...
        self._html_content += df.to_html(index=index, border=1)
        self._html_content += "\n</table>"

    def add_image(self, image_filename: str, css_class: Optional[str] = None) -> None:
        """
